(ns dott.main
  "Kotodama WASM entrypoint for the Dott clean-room actor (L4) — Clojure port.
  ...
  facts (`dott.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(def entity-specs
  [{:entity "Driver"   :plural "drivers"   :id-prefix "dott_dri"
   ...
   {:entity "Rating"   :plural "ratings"   :id-prefix "dott_rat"
...
(defn emit-facts
  "EAVT facts for one record: {\"dott.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "dott-compat" :tier tier :entities entities} 200])
